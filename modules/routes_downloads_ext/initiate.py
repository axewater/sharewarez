...
def download_file(file_location, file_size, game_uuid, file_name):
    """Legacy download function - consider using download_other instead"""
    
    # Input validation
    if not all([file_location, file_size, game_uuid, file_name]):
        log_system_event("Missing parameters in download_file request", event_type='security', event_level='warning')
        abort(400)
    
    # Validate file_location parameter
    if file_location not in ['updates', 'extras']:
        log_system_event(f"Invalid file location attempted: {file_location}", event_type='security', event_level='warning')
        flash("Invalid file location", "error")
    ...
    if not file_name or len(file_name) > 255:
        log_system_event(f"Invalid file name attempted: {file_name[:100]}", event_type='security', event_level='warning')
        flash("Invalid file name", "error")
        return redirect(url_for('games.game_details', game_uuid=game_uuid))
    
    # Validate file_size parameter - should be numeric
    if not re.match(r'^[0-9]+[kmgtKMGT]?[bB]?$', file_size):
        log_system_event(f"Invalid file size format attempted: {file_size}", event_type='security', event_level='warning')
        flash("Invalid file size format", "error")
        return redirect(url_for('games.game_details', game_uuid=game_uuid))
    ...
    try:
        ...
        if not game:
            log_system_event(f"Game not found: {game_uuid}", event_type='game', event_level='error')
        ...
        if file_location == "updates":
            ...
            if update:
                actual_file_path = update.file_path
            else:
                log_system_event(f"Update file not found for game {game_uuid}: {file_name}", event_type='game', event_level='error')
            ...
            if extra:
                actual_file_path = extra.file_path
            else:
                log_system_event(f"Extra file not found for game {game_uuid}: {file_name}", event_type='game', event_level='error')
                flash("Extra file not found", "error")
                return redirect(url_for('games.game_details', game_uuid=game_uuid))
        
        if not actual_file_path:
            log_system_event(f"Could not resolve file path for {file_location}/{file_name}", event_type='system', event_level='error')
            flash("File path could not be resolved", "error")
            return redirect(url_for('games.game_details', game_uuid=game_uuid))
        ...
        is_safe, error_message = is_safe_path(actual_file_path, allowed_bases)
        if not is_safe:
            log_system_event(f"Path validation failed for file {file_name}: {error_message}", event_type='security', event_level='warning')
            flash("Access denied.", "error")
            return redirect(url_for('games.game_details', game_uuid=game_uuid))
        
        # Check if file exists
        if not os.path.exists(actual_file_path):
            log_system_event(f"File not found on disk: {actual_file_path}", event_type='system', event_level='error')
            flash("File not found on disk", "error")
            return redirect(url_for('games.game_details', game_uuid=game_uuid))
        
        # Check for existing download request
        existing_request = db.session.execute(select(DownloadRequest).filter_by(user_id=current_user.id, file_location=actual_file_path)).scalars().first()
        ...
        if os.path.isfile(actual_file_path):
            ...
            zip_file_path = actual_file_path
        else:
            file_size_bytes = get_folder_size_in_bytes(actual_file_path)
            zip_save_path = current_app.config['ZIP_SAVE_PATH']
            sanitized_zip_name = sanitize_filename(f"{file_name}.zip")
            zip_file_path = os.path.join(zip_save_path, sanitized_zip_name)
        ...
        new_request = DownloadRequest(
            ...
            game_uuid=game_uuid,
            file_location=actual_file_path
        )
        db.session.add(new_request)
        db.session.commit()

        log_system_event(f"Download request created for file: {file_name}", event_type='game', event_level='information')
        
        # Use the secure zip_folder function
        @copy_current_request_context
        def thread_function():
            log_system_event(f"Starting zip process for file download request {new_request.id}", event_type='game', event_level='information')
            zip_folder(new_request.id, current_app._get_current_object(), actual_file_path, file_name)
        ...
        db.session.rollback()
        log_system_event(f"Error in download_file for {file_name}: {str(e)}", event_type='system', event_level='error')
        flash("An error occurred processing your request.", "error")
        return redirect(url_for('games.game_details', game_uuid=game_uuid))
